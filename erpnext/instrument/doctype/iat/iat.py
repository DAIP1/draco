...
class IaT(AccountsController):
    ...
    def clear_depreciation_schedule(self):
        ...
        depr_schedule = []
        # frappe.throw(len(start))
        for schedule in self.get('schedules'):
            # frappe.throw(schedule.finance_book_id)
            # to update start when there are JEs linked with all the schedule rows corresponding to an FB
            if len(start) == (schedule.finance_book_id):
                # frappe.throw(schedule.finance_book_id, title="shedule")
                start.append(num_of_depreciations_completed)
                num_of_depreciations_completed = 0

            # to ensure that start will only be updated once for each FB
            if len(start) == (schedule.finance_book_id):
                # frappe.throw(schedule.finance_book_id, title="schedule")
...
def make_post_gl_entry():
    ...
    tools_categories = frappe.db.get_all('Tools Category', fields = ['name', 'enable_cwip_accounting'])
...
@frappe.whitelist()
def make_iat_movement(instrument, purpose=None):
    import json
